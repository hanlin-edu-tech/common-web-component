...
const replaceEnvVersion = buildEnv => {
    return gulp
        ...
        .pipe(
            replace(/(tutor-test-apps|tutor-apps)\/app/g, match => {
                if (buildEnv === "production") {
                    return "tutor-apps/app";
                } else {
                    return "tutor-test-apps/app";
                }
            })
        )
        .pipe(gulp.dest("./"));
...
const buildJS = gulpDone => {
    Q.fcall(templateUtil.logStream.bind(templateUtil.logStream, minifyJS.bind(minifyJS, "./src/!(js)*/*.js")))
        ...
        .then(
            templateUtil.logStream.bind(templateUtil.logStream, () => {
                return gulp
                    .src("./dist/header/ehanlin-header.js", {
                        base: "./"
                    })
                    .pipe(debug({ title: "file ->" }))
                    .pipe(
                        replace(/\(jQuery\)/g, match => {
